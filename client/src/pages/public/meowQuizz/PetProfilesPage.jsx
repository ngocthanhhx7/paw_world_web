        ...
        <div className="grid gap-5 md:grid-cols-2">
          {profiles.map((profile) => (
            <article key={profile._id} className="rounded-[28px] bg-white p-6 shadow-[0_14px_0_rgba(94,62,130,0.08)]">
              ...
              <div className="mt-6 flex flex-wrap gap-3"><Link to={`/meow-quizz/ket-qua/${profile._id}`} className="rounded-full bg-[#7d58a8] px-5 py-3 text-sm font-black text-white">Xem gợi ý AI</Link><Link to={`/meow-quizz/ho-so/${profile._id}/chinh-sua`} className="inline-flex items-center gap-2 rounded-full bg-[#f7f1ff] px-5 py-3 text-sm font-bold text-[#6d4b8d]"><Edit3 size={16} /> Sửa</Link><button type="button" onClick={() => setDeleting(profile)} className="inline-flex items-center gap-2 rounded-full bg-[#fff0f0] px-5 py-3 text-sm font-bold text-[#b84b4b]"><Trash2 size={16} /> Xoá</button></div>
            </article>
          ))}
        </div>
      </div>
